...
class CameraApp(wx.Frame):
    def __init__(self):
        ...
        self.last_eye_detected_time = time.time()
    ...
    def draw_led(self, frame):
        cv2.circle(frame, (30, 30), 15, self.led_color, -1)
        
        if self.led_color == (0, 255, 255):  # Yellow
            cv2.putText(frame, "Warning", (55, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        elif self.led_color == (0, 0, 255):  # Red
            cv2.putText(frame, "Alert", (55, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        ...
        if self.camera.isOpened():
            ...
            while self.is_running:
                ...
                if eyes_detected:
                    ...
                    elapsed_time = time.time() - self.last_eye_detected_time
                    if elapsed_time > 5:
                        self.led_color = (0, 0, 255)  # Red
                    elif elapsed_time > 3:
                        self.led_color = (0, 255, 255)  # Yellow

                # Mirror the frame for visualization
                frame = cv2.flip(frame, 1)

                # Draw the LED and text after flipping the frame
                self.draw_led(frame)
